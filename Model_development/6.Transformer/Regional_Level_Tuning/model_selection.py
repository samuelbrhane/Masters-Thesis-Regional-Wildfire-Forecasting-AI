...
print(f"\n=== Evaluating {region_label.title()} Model ===")

results_csv_path = os.path.join("results", f"{region_label}_transformer_tuning_results.csv")
if not os.path.exists(results_csv_path):
    print(f"No tuning results found for {region_label}. Skipping.")
    exit()
...
for i, model_row in enumerate(top_models):
    ...
    (
        ...
        df_test, scaler_y
    ) = preprocess_data(df.copy(), params)
...
if evaluated_models:
    ...
    eval_results_path = os.path.join("results", f"{region_label}_transformer_top_models_test_eval.csv")
