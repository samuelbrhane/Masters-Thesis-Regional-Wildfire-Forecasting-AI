...
def preprocess_data(df, params):
    """
    Preprocess wildfire data for Transformer model at the regional level.
    """
    ...
    def create_sequences(X, y, month, dow, seq_length):
        X_seq, y_seq, month_seq, dow_seq = [], [], [], []
        for i in range(len(X) - seq_length):
            X_seq.append(X[i:i + seq_length])
            y_seq.append(y[i + seq_length])
            month_seq.append(month[i:i + seq_length])
            dow_seq.append(dow[i:i + seq_length])
        return (
            np.array(X_seq),
            np.array(y_seq),
            np.array(month_seq),
