...
model_dir = os.path.join(MODELS_DIR, f"{region_label}_transformer")
results_csv_path = os.path.join(RESULTS_DIR, f"{region_label}_transformer_tuning_results.csv")
os.makedirs(model_dir, exist_ok=True)
os.makedirs(RESULTS_DIR, exist_ok=True)

df = load_data(data_path=DATA_PATH)
print(f"Loaded regional data: {len(df)} rows")
...
for trial in range(243, NUM_TRIALS):
...
print(f"\nTuning complete! Results saved to {results_csv_path}")
