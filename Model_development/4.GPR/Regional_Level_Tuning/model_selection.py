...
eval_results_path = os.path.join(RESULTS_DIR, f"{group_name}_top_models_test_eval.csv")
evaluated_models_df = pd.DataFrame(evaluated_models)
evaluated_models_df.to_csv(eval_results_path, index=False)
print(f"\nTest evaluation of top regional GPR models saved to {eval_results_path}")
