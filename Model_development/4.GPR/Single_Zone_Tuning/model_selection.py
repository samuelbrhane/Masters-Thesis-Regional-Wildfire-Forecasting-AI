import os
import pandas as pd
import joblib

from src.evaluation import evaluate_model
from src.preprocessing import preprocess_data
from src.data_loading import load_data
from evaluate_saved_model import plot_model_test

RESULTS_DIR = "results"
MODELS_DIR = "models"

# Loop through zones 1 to 8
for zone_id in range(1, 9):
    print(f"\n=== Evaluating Zone {zone_id} ===")

    results_csv_path = os.path.join(RESULTS_DIR, f"zone_{zone_id}_tuning_results.csv")
    if not os.path.exists(results_csv_path):
        print(f"Skipping Zone {zone_id} — no tuning results found.")
        continue

    results_df = pd.read_csv(results_csv_path)

    top_candidates = results_df.sort_values(
        by=[
            "val_exact_match_percentage",
            "val_r2",
            "val_rmse",
            "val_mae",
            "train_exact_match_percentage"
        ],
        ascending=[False, False, True, True, False]
    ).head(50)

    df = load_data(zone_id)
    top_models = top_candidates.to_dict("records")
    evaluated_models = []

    for i, model_row in enumerate(top_models):
        model_path = model_row["model_file"]
        model_filename = os.path.basename(model_path)
        print(f"\nEvaluating model {i + 1}: {model_filename}")

        params = {
            'fire_lag': int(model_row['fire_lag']),
            'climate_lag': int(model_row['climate_lag'])
        }

        model = joblib.load(model_path)
        _, _, _, _, X_test, y_test, _ = preprocess_data(df, params)

        metrics = evaluate_model(model, X_test, y_test)
        print(f"Test metrics for {model_filename}:")
        for k, v in metrics.items():
            print(f"  {k}: {v:.4f}")

        evaluated_models.append({
            "model_filename": model_filename,
            "model_path": model_path,
            **params,
            **metrics
        })

        plot_model_test(
            zone_id=zone_id,
            model_filename=model_filename,
            model_path=model_path,
            params=params
        )

    if evaluated_models:
        eval_df = pd.DataFrame(evaluated_models)
        output_path = os.path.join(RESULTS_DIR, f"zone_{zone_id}_top_models_test_eval.csv")
        eval_df.to_csv(output_path, index=False)
        print(f"Saved evaluation results for Zone {zone_id} to {output_path}")
