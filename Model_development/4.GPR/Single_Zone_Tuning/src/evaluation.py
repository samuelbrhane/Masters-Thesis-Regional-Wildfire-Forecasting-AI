...
def evaluate_model(model, X, y):
    """
    Evaluate the Gaussian Process Regression (GPR) model.
    """
    try:
    ...
    if y_std is not None:
        metrics['mean_prediction_std'] = np.mean(y_std)
        metrics['max_prediction_std'] = np.max(y_std)
        metrics['min_prediction_std'] = np.min(y_std)
